...
def query_model(prompt: str) -> str:
    ...
    print(
        f"🔌 Запрос к: {url} | модель: {Config.MODEL_NAME} | таймаут: {Config.TIMEOUT_GEN}s"
    )
...
def safe_exec_python(
    code: str, test_cases: List[tuple], expected_name: str = None, timeout: int = 5
) -> Dict:
    ...
    try:
        ...
        return {
            ...
            "feedback": f"{passed}/{len(test_cases)} тестов пройдено",
        }
    except TimeoutError:
        return {"score": 0, "feedback": "Превышен лимит времени исполнения"}
    except Exception as e:
        return {"score": 0, "feedback": f"Runtime ошибка: {str(e)}"}
    finally:
        signal.alarm(0)
...
class Validators:
    ...
    def v2_fastapi_endpoint(output: str) -> Dict:
        ...
        return {
            "score": min(sum(1 for c in checks if c) * 20, 100),
            "feedback": "Структурная проверка эндпоинта и валидации",
        }
    ...
    def v3_sql_query(output: str) -> Dict:
        ...
        has_logic = bool(
            re.search(r"HAVING\s+COUNT\s*\(\s*\w+\s*\)\s*>\s*3", out_upper)
        ) and bool(re.search(r"WHERE\s+.*AGE\s*>\s*30", out_upper))
        score = (len(found) / len(required)) * 80 + (20 if has_logic else 0)
        return {
            "score": min(score, 100),
            "feedback": f"Найдено {len(found)}/{len(required)} конструкций. Логика: {'✅' if has_logic else '❌'}",
        }
    ...
    def v4_js_debounce(output: str) -> Dict:
        ...
        return {
            "score": min(sum(1 for c in checks if c) * 20, 100),
            "feedback": "Анализ паттерна debounce",
        }

    @staticmethod
    def v5_median_algo(output: str) -> Dict:
        has_bs = bool(
            re.search(r"(binary|log|partition|left|right|mid|bisect)", output, re.I)
        )
        cases = [
            (
                (
        ...
        dyn_res = safe_exec_python(
            output, cases, expected_name="findMedianSortedArrays", timeout=8
        )
        total = (30 if has_bs else 0) + (dyn_res["score"] * 0.7)
        return {
            "score": min(total, 100),
            "feedback": f"Бинарный поиск: {'✅' if has_bs else '❌'} | {dyn_res['feedback']}",
        }

    @staticmethod
    def v6_csv_refactor(output: str) -> Dict:
        checks = {
            "yield/generator": bool(
                re.search(r"(yield|generator|Generator|islice)", output)
            ),
            "error_handling": bool(
                re.search(r"(try|except|ValueError|KeyError|logging)", output)
            ),
            "context_manager": output.count("with open") >= 1,
            "no_memory_bloat": not bool(
                re.search(
                    r"(all_rows\s*=\s*\[\]|\.append\s*\(.*\)|writerows\s*\(\[)", output
                )
            ),
    ...
    def v7_cpp_threadpool(output: str) -> Dict:
        checks = {
            "modern_cpp": bool(
                re.search(
                    r"(C\+\+17|C\+\+20|std::jthread|std::stop_token|auto&|\[\[maybe_unused\]\])",
                    output,
                )
            ),
            "thread_safe": bool(
                re.search(
                    r"(std::mutex|std::lock_guard|std::condition_variable|std::unique_lock)",
                    output,
                )
            ),
            "priority_queue": bool(
                re.search(r"(priority_queue|emplace|push.*priority)", output, re.I)
            ),
            "graceful_shutdown": bool(
                re.search(r"(shutdown|join_all|stop.*thread|~|RAII)", output, re.I)
            ),
            "example_usage": bool(
                re.search(r"(main\(\)|ThreadPool.*pool|submit.*task)", output, re.I)
            ),
        }
        return {
            "score": sum(1 for v in checks.values() if v) * 20,
    ...
    def v8_go_rate_limiter(output: str) -> Dict:
        checks = {
            "sliding_window": bool(
                re.search(r"(sliding|window|bucket|map.*time|sync.Map)", output, re.I)
            ),
            "thread_safe": bool(
                re.search(r"(sync\.Mutex|sync\.RWMutex|atomic|chan)", output)
            ),
            "context": bool(re.search(r"context\.Context|r\.Context", output)),
            "429_retry": bool(
                re.search(r"(429|TooManyRequests|Retry-After|X-RateLimit)", output)
            ),
            "json_response": bool(
                re.search(
                    r"(json\.Marshal|w\.WriteHeader.*429|application/json)", output
                )
            ),
        }
        return {
    ...
    def v9_async_client(output: str) -> Dict:
        checks = {
            ...
            "exponential_backoff": bool(
                re.search(
                    r"(2\s*\*\s*i|exponential|backoff|wait\s*\*\s*2)", output, re.I
                )
            ),
            "retry_after": bool(
                re.search(
                    r"(Retry-After|429|sleep.*retry|headers\[.*retry)", output, re.I
                )
            ),
            "pydantic_validation": bool(
                re.search(
                    r"(pydantic|BaseModel|validate|parse_obj|model_validate)",
                    output,
                    re.I,
                )
            ),
            "timeouts": bool(
                re.search(
                    r"(timeout=|ClientTimeout|connect_timeout|read_timeout)", output
                )
            ),
        }
        return {
    ...
    def v10_unit_tests(output: str) -> Dict:
        checks = {
            "test_framework": bool(
                re.search(r"(pytest|unittest|assertEqual|@pytest\.mark|test_)", output)
            ),
            "mocking": bool(
                re.search(r"(patch|Mock|MagicMock|side_effect|return_value)", output)
            ),
            "edge_cases": bool(
                re.search(
                    r"(negative|zero|empty|conflict|VIP.*promo|boundary)", output, re.I
                )
            ),
            "isolation": output.count("def test_") >= 3,
            "coverage_hint": bool(
                re.search(
                    r"(fixture|setup|teardown|parametrize|@pytest\.param)", output
                )
            ),
        }
        return {
...
class CodingBenchmark:
    ...
    def _make_conclusion(
        ...
        lines = []
        if overall >= 90:
            tier = "🟢 ОТЛИЧНО (Production-ready)"
        elif overall >= 75:
            tier = "🟡 ХОРОШО (Требует мелких правок)"
        elif overall >= 60:
            tier = "🟠 СРЕДНЕ (Структурные недочёты, нужен рефакторинг)"
        else:
            tier = "🔴 НИЗКО (Не справляется с требованиями, много галлюцинаций)"
        ...
        if hard < 60:
            lines.append("⚠️ Слабое место: многопоточность/асинхронность/архитектура.")
        if medium < 60 and hard >= 60:
            lines.append(
                "⚠️ Проблема: провал на алгоритмах, но знает синтаксис сложных языков."
            )
        if easy < 80:
            lines.append("⚠️ Базовый синтаксис нестабилен. Высокий риск галлюцинаций.")
        if overall >= 75:
            lines.append(
            ...
            lines.append(
                "❌ Рекомендация: доработать промпты, снизить temperature, добавить RAG/контекст."
            )
        return "\n".join(lines)
